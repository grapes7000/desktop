...
def test_helpers_are_python_and_json_safe():
    ...
    assert set(scripts) == {
        ...
        , "dashboard.py", "dashboard-action.py"
    }
...
def test_dashboard_surface_has_complete_widget_states():
    yuck = theme_homepage.render_yuck({"alignment": "left"})
    assert "desktop-mode" in yuck
    assert "workspace.scrollable" in yuck
    assert "media.elapsed" in yuck
    assert "agenda.current" in yuck
    assert "network.tailscale" in yuck
    assert "project.modified" in yuck
    assert "dashboard-action.py" in yuck
    assert "Clipboard history" in yuck
    assert "Color picker" in yuck


def test_dashboard_helper_uses_safe_empty_states(tmp_path):
    script = tmp_path / "dashboard.py"
    script.write_text(theme_homepage.render_scripts()["dashboard.py"])
    env = dict(os.environ)
    env["PATH"] = str(tmp_path / "empty-path")
    env["HOME"] = str(tmp_path)

    for section in ("agenda", "network", "project", "mode"):
        result = subprocess.run(
            [sys.executable, str(script), section], env=env,
